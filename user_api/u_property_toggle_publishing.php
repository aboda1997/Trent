...
if ($pro_id == '' or $uid == '') {
...
} else if (validateIdAndDatabaseExistance($pro_id, 'tbl_property',  "  add_user_id = " . $uid . " and is_deleted =0") === false) {
	...
	$data = [["id" => $pro_id , 'is_published'=> true ]  ]; 
	...
	$h = new Estate();
	$check = $h->restateupdateData_single($field, $table, $where);
	$data = [["id" => $pro_id , 'is_published'=> false ]  ]; 
