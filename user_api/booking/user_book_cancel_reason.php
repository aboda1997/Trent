...
try {
    ...
    $pol = array();
    $c = array();
    $sel = $rstate->query("SELECT id,  JSON_UNQUOTE(JSON_EXTRACT(reason, '$.$lang_code')) AS title FROM tbl_user_cancel_reason WHERE status=1");
    ...
    if (empty($c)) {
        ...
        $returnArr    = generateResponse('true', "User Cancel Reason List Not Founded!", 200, array(
            "cancel_reason_list" => $c,
            "length" => count($c),
        ));
    } else {
        $returnArr    = generateResponse('true', "User Cancel Reason List Founded!", 200, array(
            "cancel_reason_list" => $c,
            "length" => count($c),
        ));
    }
